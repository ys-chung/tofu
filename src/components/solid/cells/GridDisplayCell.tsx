import { type Accessor, Show } from "solid-js"

export const GridDisplayCell = (props: {
  displayChar: () => string
  weight: Accessor<number>
  fontName: string
  displayFontName: string
  placeName: string
  langAttr: string
  writingSystemName: string | null
}) => {
  return (
    <div class="relative flex aspect-square flex-col justify-between bg-white p-2 text-sm">
      <div class="flex-row flex">
        <h2 class="z-10 font-semibold">{props.placeName}</h2>
        <Show when={props.writingSystemName}>
          <span class="text-stone-500">
            &nbsp;·&nbsp;{props.writingSystemName}
          </span>
        </Show>
      </div>
      <div
        class="absolute inset-0 flex select-none items-center justify-center text-7xl sm:text-9xl pointer-events-none"
        lang={props.langAttr}
        style={`font-variation-settings: 'wght' ${props.weight()}; font-family: ${props.fontName}, AdobeBlank;`}
      >
        {props.displayChar()}
      </div>
      <p class="z-10 text-right">{props.displayFontName}</p>
    </div>
  )
}
